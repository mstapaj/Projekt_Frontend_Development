...
router.get("/:id", async (req, res) => {
  ...
  const personsRows = await client.query("SELECT * FROM person WHERE id = $1", [
    id,
  ]);
...
router.post("/", async (req, res) => {
  ...
  const insertedPersonRows = await client.query(
    ...
    [
      personToAdd.first_name,
      personToAdd.last_name,
      personToAdd.birth_date,
      personToAdd.nationality,
    ]
  );
...
router.put("/:id", async (req, res) => {
  ...
  const result = await client.query(
    `UPDATE person SET first_name = $1, last_name = $2, birth_date = $3, nationality = $4 WHERE id = $5`,
    [
      personToAdd.first_name,
      personToAdd.last_name,
      personToAdd.birth_date,
